...
void FaceLocator::InitCascades(void)
{
	...
	if( (!System::IO::File::Exists(FaceCascade))
	||(!System::IO::File::Exists(LeftEyeCascade))
	||(!System::IO::File::Exists(RightEyeCascade))
	)
	...
	char* face = StringToCharPtr(FaceCascade);

	FaceDetector->InitObjectDetect(face);

	char* leye = StringToCharPtr(LeftEyeCascade);	
	LeftEyeDetector->InitObjectDetect(leye);
	
	char* reye = StringToCharPtr(RightEyeCascade);
